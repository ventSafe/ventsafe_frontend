...
export default function LoginPage() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!keyToUse) {
        ...
        const studentEncrypted    = localStorage.getItem(STORAGE_KEYS.ENCRYPTED_PRIVATE_KEY);
        const counsellorEncrypted = localStorage.getItem("ventsafe-counsellor-encrypted-key");
        const encryptedKey        = studentEncrypted || counsellorEncrypted;
      ...
      if (result.success && result.data) {
        ...
        // Always update the stored name on login — clears old account name
        ...
        // Update auth store immediately so RouteGuard sees authenticated state
        // without waiting for another checkAuth() call
        setUser(user);

        // Cross-role check — student login page should not accept counsellor keys
        ...
        if (isNewUser) {
          // New user — skip name prompt, go to welcome
          router.push("/welcome");
        } else {
          // Returning user — show name change prompt before redirecting
  ...
  return (
    <div className="min-h-screen bg-ventsafe-background flex flex-col">
        ...
        <div className="w-full max-w-md">
          ...
          >
            ...
            <p className="text-sm text-ventsafe-foreground/80">
              Enter your keys + PIN to continue.
            </p>
          </motion.div>
      ...
      {/* Name change prompt for returning users */}
      <LogoutModal
